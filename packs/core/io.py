...
def writer(path        : str,
    ...
    h5            = None
    tables_cache  = {} # Dont recreate tables each time
    rows_buffered = {}
    try:
        ...
        if f"/{group}" in h5:
            ...
            if overwrite:
                h5.remove_node(f"/{group}", recursive=True)
        else:
            # create group if it doesn't exist
            h5.create_group('/', f'{group}', f'{group} group generated by MULE')


        def write(dataset     :  str,
                  data        :  np.ndarray,
                  data_class  :  Type[tb.IsDescription],
                  chunk_size  :  int = 1000,
                  num_rows    :  Optional[int] = None,
                  filters     :  Optional[tb.Filters] = None) -> None:
            '''

            '''
            # collect relevant table, or create if nonexistent
            if dataset not in tables_cache:
                table = h5.create_table(group, dataset, data_class, f"Table {dataset} for group {group} generated by MULE")
                tables_cache[dataset]  = table
                rows_buffered[dataset] = 0
            else:
                table                  = tables_cache[dataset]

            # assign row
            row = table.row

            # create the bufferer here!

    finally:
        # if h5 is open, make sure it closes.
        # done to reduce code complexity
